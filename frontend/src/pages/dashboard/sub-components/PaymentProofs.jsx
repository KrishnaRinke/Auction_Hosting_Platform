...
const PaymentProofs = () => {
  ...
  const handlePaymentProofDelete = (id) => {
    dispatch(deletePaymentProofs(id));
  };
  ...
  return (
    <>
     ...
     {openDrawer && singlePaymentProof && Object.keys(singlePaymentProof).length > 0 && (
...
const handlePaymentProofUpdate = () => {
  ...
  <section
  className={`fixed inset-0 z-50 bg-black bg-opacity-50 flex items-end transition-all duration-300 ${
    openDrawer && singlePaymentProof?.userId ? "visible opacity-100" : "invisible opacity-0"
  }`}
>
        <div className="bg-white max-h-[90vh] overflow-y-auto transition-all duration-300 w-full sm:max-w-[640px] sm:mx-auto">
          <div className="w-full px-5 py-8 ">
            ...
            <form className="flex flex-col gap-5 my-5">
              <div className="flex flex-col gap-3">
                ...
                <input type="text" value={singlePaymentProof.userId || ""} disabled onChange={(e)=> e.target.value} className='text-xl px-1 py-2 bg-transparent border-[1px] border-stone-600 rounded-md focus:outline-none'/>
              </div>
              <div className="flex flex-col gap-3">
                <label className="text-[16px] text-stone-600">
                  Amount
               ...
               <select value={status} onChange={(e)=>setStatus(e.target.value)} className='text-xl px-1 py-2 bg-transparent border-[1px] border-stone-600 rounded-md focus:outline-none'>
              ...
              <div className="flex flex-col gap-3">
                ...
                <textarea rows={5} value={singlePaymentProof.comment || ""} onChange={(e)=>e.target.value} disabled className='text-xl px-1 py-2 bg-transparent border-[1px] border-stone-600 rounded-md focus:outline-none'/>
              </div>
              <div>
                <Link to={singlePaymentProof.proof?.url || ""} className="bg-[#D6482B] flex justify-center w-full py-2 rounded-md text-white font-semibold text-xl transition-all duration-300 hover:bg-[#b8381e]" target="_blank">Payment Proof (ss)</Link>
              </div>
              <div>
                <button
                  type="button"
