...
const auctionSlice = createSlice({
    ...
    reducers:{
       ...
       createAuctionFailed(state,action){
        ...
        getAllAuctionItemFailure(state,action){
            state.loading = false
        },
...
export const getAllAuctionItems = () => async (dispatch) =>{
    ...
    try{
    const response = await axios.get("http://localhost:4000/api/v1/auctionitem/allitems",{withCredentials:true});
    dispatch(auctionSlice.actions.getAllAuctionItemSuccess(response.data.items));
    dispatch(auctionSlice.actions.resetSlice());
    } catch (error){
       dispatch(auctionSlice.actions.getAllAuctionItemFailure());
       console.error(error);
           dispatch(auctionSlice.actions.resetSlice());
    }
};
...
export const getMyAuctionItems = () => async (dispatch) =>{
    ...
    try{
    const response = await axios.get("http://localhost:4000/api/v1/auctionitem/myitems",{withCredentials:true});
...
export const getAuctionDetail = (id) => async (dispatch) =>{
    ...
    try{
    const response = await axios.get(`http://localhost:4000/api/v1/auctionitem/auction/${id}`,{withCredentials:true});
...
export const createAuction = (data) => async(dispatch)=>{
    ...
    try {
      const response = await axios.post("http://localhost:4000/api/v1/auctionitem/create",data,{withCredentials:true,headers:{"Content-Type":"multipart/form-data"},});
...
export const republishAuction = (id,data)=>async(dispatch)=>{
   ...
   try {
    const response = await axios.put(`http://localhost:4000/api/v1/auctionitem/item/republish/${id}`,data,{
...
export const deleteAuction = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `http://localhost:4000/api/v1/auctionitem/delete/${id}`,
      {
        withCredentials: true,
      }
    );
