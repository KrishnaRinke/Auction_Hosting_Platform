...
export const addNewAuctionItem = catchAsyncErrors(async(req,res,next)=>{
    ...
    if(
        ...
        return next(new ErrorHandler("Please provide all deatils of the item",400))
    }
    if(new Date(startTime) < Date.now()){
        return next(new ErrorHandler("Auction starting time must be greater than the present time",400));
    }
    if(new Date(startTime) >= new Date(endTime)){
        return next(new ErrorHandler("Auction starting time must be lesser than the ending time",400));
    }
   ...
   try{
    const cloudinaryResponse = await cloudinary.uploader.upload(image.tempFilePath,{
      ...
      if(!cloudinaryResponse || cloudinaryResponse.error){
        ...
        return next(new ErrorHandler("Failed to upload auction image to the cloudinary",500));
      }
      const auctionItem = await Auction.create({
        title,description, category , condition, startingBid ,startTime,endTime,image:{
      ...
      return res.status(201).json({
        success:true,
        message:`Auction item created and it will be listed on auction page at ${startTime}`,
        auctionItem,
      }) 
   }catch (error){
     return next(new ErrorHandler(error.message || "Failed to create the auction",500))
   }
...
export const getMyAuctionsItems = catchAsyncErrors(async(req,res,next)=>{
